...
void RTWMessage( int Lev, char s1[], char s2[], char s3[],
  ...
  sprintf( title, "InTeCo/2K®™ RT-CON for Windows /  %s", MODEL_NAME_STR );
  if( Lev > 0 )
    MessageBox( ( HWND) NULL, (LPCSTR) aux, (LPCSTR) title, (UINT) (MB_OK + +MB_SYSTEMMODAL+MB_ICONINFORMATION+MB_ICONSTOP) );
  else
    MessageBox( ( HWND) NULL, (LPCSTR) aux, (LPCSTR) title, (UINT) MB_OK +MB_SYSTEMMODAL+MB_ICONINFORMATION);
}
