...
#define S_FUNCTION_NAME rtdacusb2_tras_ddd
...
static void mdlInitializeSizes(SimStruct *S) {
    ...
    ssSetOutputPortWidth(S,  5,  1 );            // AzimuthCurr
    ssSetOutputPortWidth(S,  6,  1 );            // PitchCurr
    ssSetOutputPortWidth(S,  7,  1 );            // TmrCounter
    ssSetOutputPortWidth(S,  8,  2 );            // ThermStatus
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    real_T *yAzimuthCurr    = ssGetOutputPortRealSignal(S,  5);
    real_T *yPitchCurr      = ssGetOutputPortRealSignal(S,  6);
    real_T *yTmrCounter     = ssGetOutputPortRealSignal(S,  7);
    real_T *yThermStatus    = ssGetOutputPortRealSignal(S,  8);
    ...
    for(i=0;i<16;i++) RTDACUSBBuffer.AD[i].Gain = 1;   // Gain 2
    ...
    yPitchCurr[0] = RTDACUSBBuffer.AD[2].Result & 0x0FFF;
    if( yPitchCurr[0] >= 2048 ) yPitchCurr[0] -= 4096;
    yPitchCurr[0] = 10.0*(yPitchCurr[0]/2048.0/2.0);
    yAzimuthCurr[0] = RTDACUSBBuffer.AD[3].Result & 0x0FFF;
    if( yAzimuthCurr[0] >= 2048 ) yAzimuthCurr[0] -= 4096;
    yAzimuthCurr[0] = 10.0*(yAzimuthCurr[0]/2048.0/2.0);
#endif    
}
